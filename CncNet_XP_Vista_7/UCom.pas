...
uses
  SysUtils, UType, IniFiles, Windows;
...
var
  ...
  portAdresseBase : word; // Adresse de base du port parallèle lue dans le fichier ini
  portAdresseBase_string : string; // Adresse de base du port parallèle qui est lue dans le fichier ini et s'affiche dans Getdescription
  nom_port : string; // nom du port parallèle, non utilisé.
...
begin
  Description := 'Protocole pour interfaces parallèles, compatible Windows XP/Vista/7 v1.6. Chauffe & timer externe. Adresse port : ' + portAdresseBase_string;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        horloge := Inp32(portAdresseBase+1);
        ...
        begin
          ...
          begin
            ...
            Out32(portAdresseBase, bitSens);
          end;
          if(pulse=vitesse) then
          begin
            // Emettre l'ordre de rotation + sens
            Out32(portAdresseBase, bitRotation + bitSens);
          end;
...
type
  TContrainte = record
    byte, iDernier, compteur : integer;
  end;
var
...
begin
  ...
  // Une log par moteur, et 2 log spéciales pour les mouvements par paire de moteur et non lissé ("X1 + X2" ou "Y1 + Y2")
  // Donc comme si on avait 6 moteurs, donc 6 "contraintes"
  for j:=0 to 5 do
  begin
    Contrainte[j].byte := 0;
    Contrainte[j].iDernier := -1;
    Contrainte[j].compteur := 0;
  end;

  nbOrdres := Length(ArrayOrdres.ArrayOrdres)-2;
  for i:=0 to nbOrdres do
  begin
    // Tester si un ordre sur ce moteur a déjà été traité par cette boucle
    for j:=0 to 5 do
    begin
      // Si on le trouve on sort de la boucle
      if (Contrainte[j].byte=ArrayOrdres.ArrayOrdres[nbOrdres - i].bitRotation) then
      begin
      	if (Contrainte[j].compteur<ArrayOrdres.ArrayOrdres[nbOrdres - i].vitesse) then
        begin
          ArrayOrdres.ArrayOrdres[nbOrdres - i].vitesse := ArrayOrdres.ArrayOrdres[nbOrdres - i].vitesse - Contrainte[j].compteur;
        end else begin
          ArrayOrdres.ArrayOrdres[nbOrdres - i].vitesse := 0;
        end;
        Contrainte[j].compteur := 0;
        for k := 0 to 5 do
        begin
          if (Contrainte[k].byte <> ArrayOrdres.ArrayOrdres[nbOrdres - i].bitRotation) then
          begin
            Contrainte[k].compteur := Contrainte[k].compteur + ArrayOrdres.ArrayOrdres[nbOrdres - i].vitesse;
          end;
        end;

        break;
      end else begin
      	// Sinon on initialise le tableau des contraintes pour ce moteur et on sort de la boucle
        if(Contrainte[j].byte=0) then
        begin
          Contrainte[j].byte := ArrayOrdres.ArrayOrdres[nbOrdres - i].bitRotation;
          Contrainte[j].compteur := 0;
          ArrayOrdres.ArrayOrdres[nbOrdres - i].vitesse := 0;
          break;
        end;
...
{-----------------------------------------------------------------------------------------------------------------------
{Lecture des informations du port dans le adresse_port_LPT.ini}
begin
  Fichier := Tinifile.Create(ExpandFileName('adresse_port_LPT.ini'));
  portAdressebase_string := Fichier.ReadString('portLPT','adresse', '/!\');
  nom_port := Fichier.ReadString('portLPT','nom', '/!\');
  Fichier.free; // on libère
  portAdressebase := StrtoInt('$' + portAdressebase_string);
